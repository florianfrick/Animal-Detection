...
# TransformerDecoder for object detection (DETR-style)
class TransformerDecoder(nn.Module):
    ...
    def forward(self, memory):
        ...
        tgt = torch.zeros_like(query_embed)  # (B, n_queries, emb_dim)
...
class DetectionHead(nn.Module):
    def __init__(self, emb_dim, n_classes):
        super().__init__()
        self.class_head = nn.Linear(emb_dim, n_classes+1)
        self.bbox_head = nn.Linear(emb_dim, 4)  # [cx, cy, w, h] normalized

    def forward(self, x):
        # x: (B, n_queries, emb_dim)
        class_logits = self.class_head(x)  # (B, n_queries, n_classes)
        bboxes = self.bbox_head(x).sigmoid()  # (B, n_queries, 4), normalized [0,1]
        return class_logits, bboxes


# Full encoder-decoder detection model
class VisionDetectionModel(nn.Module):
    def __init__(self, img_size, patch_size, in_channels, emb_dim, n_heads, n_enc_layers, n_dec_layers, n_queries, n_classes, dropout=0.1):
        super().__init__()
        ...
        self.detection_head = DetectionHead(emb_dim, n_classes)
